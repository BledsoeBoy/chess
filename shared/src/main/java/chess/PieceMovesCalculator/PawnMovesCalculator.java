...
public class PawnMovesCalculator {
    public static void pieceMoves(ChessBoard board, ChessPosition position, Collection<ChessMove> moves) {
        ...
        if (direction == 1) {
            addPawnAttackLeft(board, position, row, col, 1, 1, direction, moves);
            addPawnAttackRight(board, position, row, col, 1, -1, direction, moves);
        }
        else if (direction == -1) {
            addPawnAttackLeft(board, position, row, col, -1, 1, direction, moves);
            addPawnAttackRight(board, position, row, col, -1, -1, direction, moves);
    ...
    private static void addPawnAttackLeft(ChessBoard board, ChessPosition position, int startRow, int startCol, int rowDirection, int colDirection, int direction, Collection<ChessMove> moves) {
    ...
    private static void addPawnAttackRight(ChessBoard board, ChessPosition position, int startRow, int startCol, int rowDirection, int colDirection, int direction, Collection<ChessMove> moves) {
        int row = startRow + rowDirection;
        int col = startCol + colDirection;
